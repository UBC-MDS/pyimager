...
def test_redusize():
    ...
    #  testing that the funciton raises exception error when the the width larger than the original width
    try:
        redusize(
        ...
        assert False, f'AssertionError should be raised. \
            Width of must be less than the origial width'
    #  testing that the funciton raises exception error when the the height larger than the original height
    try:
        redusize(
        ...
        assert False, f'AssertionError should be raised. \
            Height of must be less than the origial height'
    redusize(
        "images/mandrill.jpg", "images/reduced_mandrill.jpg", 200, 200)
    #  testing that the funciton writes the image to the provided output path
    assert os.path.exists(
