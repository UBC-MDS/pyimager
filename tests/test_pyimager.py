...
def test_img_filter():
    ...
    img_filter('images/mandrill.jpg', filter_type='blur',
               strength=0.1, output_path='images/mandrill_new.jpg')
    assert os.path.exists(
        'images/mandrill_new.jpg'), 'File should be saved to the provided output path'
    os.remove('images/mandrill_new.jpg')

    # test that if output_path is None the file is not saved
    output_test = img_filter(
        'images/mandrill.jpg', filter_type='blur', strength=0.1, output_path=None)
    assert os.path.exists(
        'images/mandrill_new.jpg') == False, 'File should not be saved'
